...
class ExerciseDialogueSystem:
    ...
    def _get_system_prompt(self, user_id: str) -> str:
        ...
        base_prompt = """You are a knowledgeable and empathetic exercise companion for older adults."""
        if self.user_memory.has_user_memory(user_id):
            user_info = self.user_memory.get_user_memory(user_id)
            memory_prompt = f"""
            Previous information about this user:
            - Age: {user_info.get('age')}
            - Health Conditions: {', '.join(user_info.get('health_conditions', []))}
            - Mobility Level: {user_info.get('mobility_level')}
            - Exercise Preferences: {', '.join(user_info.get('exercise_preferences', []))}
            - Safety Concerns: {', '.join(user_info.get('safety_concerns', []))}
            
            Please acknowledge this existing information and ask if anything has changed or if they have new preferences to share.
            """
